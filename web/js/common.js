...
var checkPasswd = function(id, pass, rtnMsg) {
	...
	// 조건. 영문 대소문자 구분 &숫자&특수문자 3가지 조합 10자리 이상으로 설정하셔야 합니다.
	//var exp_sp = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{10,}$/;

	// 조건. 영문&숫자&특수문자 3가지 조합 10자리 이상으로 설정하셔야 합니다.
	var exp_sp = /^(?=.*?[A-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{10,}$/;
	...
	else if (!exp_sp.test(pass)) {
		msg = "영문&숫자&특수문자 3가지 조합 10자리 이상으로 설정하셔야 합니다.";
	}
	else 
	{
