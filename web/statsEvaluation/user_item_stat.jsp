...
<script type="text/javascript">
	...
	$(function () 
	{
		...
		baseObj.toolbar.items.push(
				...
				{ type:	"button", icon:	"ui-icon-excel", label:	"엑셀", style: 'float: right;	margin-right: 5px;', listeners:	[{
						"click": function () {
							...
							var args = {"itemData":objToStr(itemData)};
							makeFormAndGo("fExcel", "excel_<%=page_id%>.jsp?"+$("#search").serialize(), "hiddenFrame", args);
					}
				}]
			}
		);
		...
		*/
		...
		obj.refresh = function (evt, ui) {
			...
			//if(data==undefined || data.length==0) return;	총계 이전정보 남아있는 현상 발생 CJM 20180511
	...
	function goStats(kind)
	{
		...
		{
			document.location = "user_comment_stat.jsp";
		}
	}
		...
		<div class="panel-heading">
			<div class="panel-options">
				<ul class="nav nav-tabs">
					...
					<li class=""><a href="javascript:goStats('comment')">상담원별 의견</a></li>
				</ul>
			</div>
		</div>
	</div>
