...
<%
	...
	try 
	{
		...
		if(pcipCnt == 0)
		{
			Site.writeJsonResult(out, false, "해당 PC의 IP ( " + login_ip + " ) 가 \\n접근허용 되지않아 접근하실수 없습니다. 관리자에게 문의 바랍니다");
			return;
		}
		//out.print("login_ip : " + login_ip + "pcipCnt : " + pcipCnt);

			
		// get parameter
		String rec_datm = CommonUtil.ifNull(request.getParameter("date"));
		String rec_keycode = CommonUtil.ifNull(request.getParameter("key"));
		...
		// 파라미터 체크
		if(!CommonUtil.hasText(rec_keycode) || !CommonUtil.hasText(rec_datm) || !CommonUtil.hasText(local_no)) 
		{
